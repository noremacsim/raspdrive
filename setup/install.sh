...
then
  ...
  then
    ...
    then
      ...
      then
        ...
        echo "initramfs initrd.img-$(uname -r) followkernel # RASPDRIVE-REMOVE" >> /boot/config.txt
      else
    ...
    {
      while ! curl -s https://raw.githubusercontent.com/noremacsim/raspdrive/main/tools/debian-resizefs.sh
      do
        sleep 1
      done
    } | bash -s 3G
  ...
  if [ -e /boot/config.txt ] && grep -q RASPDRIVE-REMOVE /boot/config.txt
  then
    # switch Raspberry Pi OS back to not using initramfs
    sed -i '/RASPDRIVE-REMOVE/d' /boot/config.txt
...
then
  while ! curl -o /boot/raspdrive_setup_variables.conf https://raw.githubusercontent.com/noremacsim/raspdrive/main/pi_build/00-raspdrive-tweaks/files/raspdrive_setup_variables.conf.sample
  do
    sleep 1
  done
fi
...
then
  while ! curl -o /boot/wpa_supplicant.conf.sample https://raw.githubusercontent.com/noremacsim/raspdrive/main/pi_build/00-raspdrive-tweaks/files/wpa_supplicant.conf.sample
  do
    sleep 1
  done
fi
...
rm -f /etc/rc.local
while ! curl -o /etc/rc.local https://raw.githubusercontent.com/noremacsim/raspdrive/main/pi_build/00-raspdrive-tweaks/files/rc.local
do
  sleep 1
done
chmod a+x /etc/rc.local
